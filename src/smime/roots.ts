...
export const trustedCertificates: pkijs.Certificate[] = [];

roots.forEach((item) => {
  try {
    const buf = new TextEncoder().encode(item).buffer;
    trustedCertificates.push(...parseCertificate(buf));
  } catch (e) {
    console.error(`Incorrect certificate data: ${item}`);
  }
});
